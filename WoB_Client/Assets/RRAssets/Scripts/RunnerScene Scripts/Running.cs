...
namespace RR {
	public class Running : MonoBehaviour {
		...
		void Start () {
	        ...
	        gameObject.GetComponent<RRMessageQueue>().AddCallback(Constants.SMSG_AUTH, ResponseLogin);
	...
	//		pcontroller[0].speed = originalSpeed + boost;
		...
		void Update () {
			...
			if (!called) {
				mainObject.GetComponent<RRMessageQueue>().AddCallback(Constants.SMSG_RRENDGAME, ResponseRREndGame);
