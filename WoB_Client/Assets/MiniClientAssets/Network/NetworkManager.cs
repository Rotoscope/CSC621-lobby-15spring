...
public class NetworkManager : MonoBehaviour {
	...
	// Connection
	private ConnectionManager connectManager = new ConnectionManager();
	...
	void Start() {
		...
		if (connectManager.Connect(Config.REMOTE_HOST, Constants.REMOTE_PORT) == ConnectionManager.SUCCESS) {
			NetworkManager.Send(
	...
	void Update () {
		...
		while (requests.Count > 0) {
			...
			if (connectManager.Send(packet.GetBytes())) {
				requests.Dequeue();
	...
	private IEnumerator Poll(float time) {
		while (true) {
			if (connectManager.Connected) {
				connectManager.Send(HeartbeatProtocol.Prepare().GetBytes());
